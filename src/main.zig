...
pub fn main() !void {
    ...
    switch (day) {
        ...
        16 => {
            switch (part) {
                1 => std.debug.print("Answer is {d}\n", .{try day16.part1(allocator, data.lines)}),
                2 => std.debug.print("Answer is {d}\n", .{try day16.part2(allocator, data.lines)}),
                else => std.debug.print("Unknown / not ready implementation for part {d}\n", .{part}),
            }
        },
        17 => {
            switch (part) {
                1 => std.debug.print("Answer is {d}\n", .{try day17.part1(allocator, data.lines)}),
                2 => std.debug.print("Answer is {d}\n", .{try day17.part2(allocator, data.lines)}),
                else => std.debug.print("Unknown / not ready implementation for part {d}\n", .{part}),
            }
        },
        18 => {
            switch (part) {
                1 => std.debug.print("Answer is {d}\n", .{try day18.part1(allocator, data.lines)}),
                2 => std.debug.print("Answer is {d}\n", .{try day18.part2(allocator, data.lines)}),
                else => std.debug.print("Unknown / not ready implementation for part {d}\n", .{part}),
            }
        },
        19 => {
            switch (part) {
                1 => std.debug.print("Answer is {d}\n", .{try day19.part1(allocator, data.lines)}),
                2 => std.debug.print("Answer is {d}\n", .{try day19.part2(allocator, data.lines)}),
                else => std.debug.print("Unknown / not ready implementation for part {d}\n", .{part}),
            }
        },
        20 => {
            switch (part) {
                1 => std.debug.print("Answer is {d}\n", .{try day20.part1(allocator, data.lines)}),
                2 => std.debug.print("Answer is {d}\n", .{try day20.part2(allocator, data.lines)}),
                else => std.debug.print("Unknown / not ready implementation for part {d}\n", .{part}),
            }
        },
